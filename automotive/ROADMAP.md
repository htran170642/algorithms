...
| 6 | CAN-FD + DBC | `av_can`: `DbcDatabase` parser (`BO_`/`SG_`/`CM_`/`VAL_`, bit-31 extended flag, `[min\|max]` enforced) · [`dbc/cockpit.dbc`](dbc/cockpit.dbc) with all 7 signals of CLAUDE.md §7 · `apps/common/` deleted — both apps read the one file | — | [`w06_dbc.md`](notes/w06_dbc.md) | [x] |
