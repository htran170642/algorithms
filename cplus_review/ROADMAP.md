...
- [x] **W44 (artifact)** — SIMD awareness · autovectorization · `-march` · reading asm — `phase06_vectorize_test` (3 kernels in their own TU) + evidence from the compiler itself. MEASURED: `-O2` vectorizes NOTHING (GCC13 very-cheap cost model) → `-O3` needed; `-O` decides *whether*, `-march` decides *width* (`-msse2`=16B/`addps`/xmm vs `-march=native`=32B/`vaddps`/ymm); `__restrict` drops the aliasing guard (add_aliased 79 instr/19 branch → add_restrict 56/11); `dot` default vectorizes the *multiply* (`vmulps`) but keeps a **scalar in-order sum** (`vaddss`) — only `-ffast-math` re-associates into a packed reduction (`vaddps`+`vfmadd231ps`). Roofline: SIMD is useless when memory-bound. Green under -20/-23. → notes/simd_vectorization.md
  - [ ] **W44 remainder** — **Mock #6** + **Design round #6** (closes Phase 6)
